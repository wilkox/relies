...
package Node {
    ...
    has old_descendants => (
        ...
        traits => ['Array'], 
        handles => {
            has_old_descendants => 'count', 
            all_old_descendants => 'elements'
        }, 
        builder => '_build_old_descendants', 
        lazy => 1
    );
    ...
    has young_ancestors => (
        ...
        traits => ['Array'], 
        handles => {
            has_young_ancestors => 'count', 
            all_young_ancestors => 'elements'
        }, 
        builder => '_build_young_ancestors', 
        lazy => 1
    );
    ...
    sub _build_descendants {
        ...
        foreach my $childGitPath ($self->children) {
            ...
            $descendants{$_}++ for $node{$childGitPath}->descendants;
        }
...
if (@parents || @bereaved) {
    ...
    &remove_parents($_, @bereaved) for @fileList;
